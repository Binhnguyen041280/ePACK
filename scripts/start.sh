...
# Load images from tar files if needed
if ! docker images | grep -q "epack-backend"; then
    if [ -f images/epack-backend.tar ]; then
        echo -e "${YELLOW}📦 Loading backend image from tar...${NC}"
        docker load -i images/epack-backend.tar
        echo -e "${GREEN}✅ Backend image loaded${NC}"
    else
        echo -e "${RED}❌ Backend image not found. Please ensure images/epack-backend.tar exists.${NC}"
        exit 1
    fi
fi

if ! docker images | grep -q "epack-frontend"; then
    if [ -f images/epack-frontend.tar ]; then
        echo -e "${YELLOW}📦 Loading frontend image from tar...${NC}"
        docker load -i images/epack-frontend.tar
        echo -e "${GREEN}✅ Frontend image loaded${NC}"
    else
        echo -e "${RED}❌ Frontend image not found. Please ensure images/epack-frontend.tar exists.${NC}"
        exit 1
    fi
fi
